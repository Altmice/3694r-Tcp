...
bool CTCPClient::Status3964R=true;
//析构函数
CTCPClient::~CTCPClient()
{
	//关闭线程退出事件句柄
...
DWORD CTCPClient::SocketThreadFunc(LPVOID lparam)
{
	...
	while (TRUE)
	{
		...
		if (ret > 0)
		{
			...
			{
				ZeroMemory(pSocket->m_recvBuf,10);
				//接收数据
				int recvLen = recv(pSocket->m_socket,pSocket->m_recvBuf,BufSize,0); 
...
-----------------------------------------------------------------*/
BOOL CTCPClient::Open(void * pOwner)
{
	//复位线程退出事件
...
------------------------------------------------------------------*/
BOOL CTCPClient::SendData(const char * buf , DWORD dwBufLen)
{
	int nBytes = 0;
	int nSendBytes=0;
